...
fn migrate_legacy_user_data(app: &tauri::AppHandle) {
  let Some(current_dir) = app_data_dir(app) else { return; };
...
fn save_settings_internal(app: &tauri::AppHandle, settings: &Settings) {
  if let Some(path) = settings_path(app) {
    ensure_parent(&path);
    let _ = fs::write(path, serde_json::to_string_pretty(settings).unwrap_or_else(|_| "{}".to_string()));
  }
}

fn is_ru(app: &tauri::AppHandle) -> bool {
  load_settings_internal(app).language.eq_ignore_ascii_case("ru")
}

fn load_index_internal(app: &tauri::AppHandle) -> IndexData {
  let Some(path) = index_path(app) else { return IndexData::default(); };
  if !path.exists() {
...
fn find_rust_indexer_path(app: &tauri::AppHandle) -> Option<PathBuf> {
  ...
  let bin = if cfg!(target_os = "windows") { "rust-indexer.exe" } else { "rust-indexer" };
...
fn start_indexing_internal(app: &tauri::AppHandle, state: &AppState, force_restart: bool) -> Value {
  let ru = is_ru(app);
  let has_worker = state.worker.lock().ok().and_then(|w| w.as_ref().cloned()).is_some();
  if has_worker && !force_restart {
    return state.index_status.lock().map(|s| s.clone()).unwrap_or_else(|_| json!({"state":"indexing"}));
  }

  if !force_restart {
    let existing = index_status_from_file(app);
    if existing.get("state").and_then(|x| x.as_str()) == Some("ready")
      && existing.get("fileCount").and_then(|x| x.as_i64()).unwrap_or(0) > 0
    {
...

  {
    ...
    thread::spawn(move || {
      ...
      let current_state = state_clone
        ...
        .lock()
        .ok()
        .and_then(|s| s.get("state").and_then(|x| x.as_str()).map(|x| x.to_string()))
      ...
      if code == 0 {
        ...
        let err = json!({"state":"error","lastError":format!("Rust indexer exit code {code}")});
...
fn load_ai_status_internal(app: &tauri::AppHandle, state: &AppState) {
  let settings = load_settings_internal(app);
  let model = if settings.ai_model.trim().is_empty() { "qwen2.5:1.5b".to_string() } else { settings.ai_model.clone() };
      ...
      if let Ok(raw) = fs::read_to_string(path) {
        if let Ok(v) = serde_json::from_str::<Value>(&raw) {
          if let Some(obj) = v.as_object() {
            for (k, val) in obj { payload[k] = val.clone(); }
          }
        }
      }
    }
...
fn save_ai_status_internal(app: &tauri::AppHandle, state: &AppState) {
  let Some(path) = ai_state_path(app) else { return; };
  ensure_parent(&path);
  if let Ok(s) = state.ai_status.lock() {
    let _ = fs::write(path, serde_json::to_string_pretty(&*s).unwrap_or_else(|_| "{}".to_string()));
  }
}
...
fn stem_token(token: &str) -> String {
  let endings = [
    "иями", "ями", "ами", "его", "ого", "ому", "ему", "ыми", "ими", "иях", "иях", "ией", "ией", "ия", "ья",
    "ий", "ый", "ой", "ая", "ое", "ые", "ов", "ев", "ам", "ям", "ах", "ях", "а", "я", "у", "ю", "е", "ы", "и",
  ];
...
fn category_from_item(item: &Value) -> String {
  ...
  let stop: std::collections::HashSet<&str> = [
    "and", "the", "for", "with", "this", "that", "from", "file", "doc", "pdf", "xls", "xlsx",
    "как", "что", "для", "или", "это", "файл", "документ", "отчет", "отчёт", "данные", "информация",
  ]
  .iter()
  .copied()
  .collect();
...
fn unique_target_path(target: &Path) -> PathBuf {
  ...
  let parent = target.parent().map(|x| x.to_path_buf()).unwrap_or_else(|| PathBuf::from("."));
  let stem = target.file_stem().and_then(|x| x.to_str()).unwrap_or("file");
...
fn collect_context_from_index(app: &tauri::AppHandle, state: &AppState, query: &str, max_items: usize) -> Vec<Value> {
  ...
  for (_k, item) in index.files {
    let name = item.get("name").and_then(|x| x.as_str()).unwrap_or("").to_lowercase();
    let p = item.get("path").and_then(|x| x.as_str()).unwrap_or("").to_lowercase();
    let text_raw = sanitize_process_input(item.get("text").and_then(|x| x.as_str()).unwrap_or(""));
    ...
    scored.push((score.round() as i64, json!({
...
fn is_file_task_query(query: &str) -> bool {
  ...
  ["найд", "поиск", "собер", "документ", "файл", "из файлов", "по файлам", "сформируй", "создай", "отчет", "отчёт", "информац", "данн", "фио", "ученик", "человек"]
    .iter()
    .any(|k| q.contains(k))
}
...
fn quick_chat_reply(query: &str) -> Option<String> {
  ...
  if ["привет", "здравств", "hello", "hi", "добрый день", "добрый вечер"].iter().any(|k| q.contains(k)) {
  ...
  if q.contains("thank") {
    return Some("You're welcome. You can ask what to find in files or what final document to generate.".to_string());
  }
...
fn detect_create_file_intent(query: &str) -> Option<String> {
  ...
  if q.contains("md") || q.contains("markdown") { return Some("md".to_string()); }
  if q.contains("txt") || q.contains("текст") || q.contains("text") { return Some("txt".to_string()); }
  if q.contains("excel") || q.contains("xlsx") || q.contains("xls") || q.contains("таблиц") || q.contains("эксел") || q.contains("ексель") {
    return Some("xlsx".to_string());
  }
  if q.contains("pdf") || q.contains("пдф") { return Some("pdf".to_string()); }
  if q.contains("word") || q.contains("docx") || q.contains("doc") || q.contains("ворд") || q.contains("документ") {
...
fn should_create_empty_file(query: &str) -> bool {
  ...
  let stop: std::collections::HashSet<&str> = [
    "создай", "создать", "сформируй", "сформировать", "сделай", "файл", "документ", "док", "документ", "ворд",
    "word", "doc", "docx", "excel", "xlsx", "xls", "таблицу", "таблица", "pdf", "пдф", "txt", "text", "текст",
    "markdown", "md", "пожалуйста", "плиз", "мне", "нужен", "нужна", "нужно", "и", "в", "на", "для", "please",
    "make", "create", "generate", "save", "сохрани"
  ].iter().copied().collect();
...
fn create_file_on_desktop(ext: &str, content: &str) -> Result<PathBuf, String> {
  ...
  let file_name = format!("AI_Result_{}.{}", now_stamp(), ext);
  let out = desktop.join(file_name);
  fs::write(&out, content).map_err(|e| e.to_string())?;
  Ok(out)
}

fn read_user_file_context(file_path: &str) -> Value {
  ...
  if ext == "txt" || ext == "md" {
    if let Ok(raw) = fs::read_to_string(&p) {
      let part = sanitize_process_input(&raw).chars().take(1200).collect::<String>();
...
fn get_app_update_status(state: State<'_, AppState>) -> Value {
  state.app_update_status.lock().map(|x| x.clone()).unwrap_or_else(|_| json!({
    "state": "idle",
    "available": false,
    "version": "",
    "downloading": false,
...
async fn check_app_update_internal(app: &tauri::AppHandle, state: &AppState, manual: bool) -> Value {
  let st = load_settings_internal(app);
  let is_ru = st.language.eq_ignore_ascii_case("ru");

  let _ = set_and_emit_update_status(app, state, json!({
    "state": "checking",
    "available": false,
    "version": "",
    "downloading": false,
...
#[tauri::command]
async fn check_app_update(app: tauri::AppHandle, state: State<'_, AppState>, manual: Option<bool>) -> Result<Value, String> {
  let app_state = state.inner().clone();
  Ok(check_app_update_internal(&app, &app_state, manual.unwrap_or(false)).await)
}

#[tauri::command]
async fn install_app_update(app: tauri::AppHandle, state: State<'_, AppState>) -> Result<Value, String> {
  let app_state = state.inner().clone();
  let st = load_settings_internal(&app);
  let is_ru = st.language.eq_ignore_ascii_case("ru");
  ...
  let updater = match app.updater() {
    ...
    Err(e) => {
      ...
      return Ok(set_and_emit_update_status(&app, &app_state, json!({
        "state": "error",
        "available": false,
        "version": "",
        "downloading": false,
  ...
  let version = update.version.to_string();
  let _ = set_and_emit_update_status(&app, &app_state, json!({
    "state": "downloading",
    "available": true,
    "version": version,
    "downloading": true,
  ...
  match out {
    ...
    Err(e) => {
      ...
      Ok(set_and_emit_update_status(&app, &app_state, json!({
        "state": "error",
        "available": true,
        "version": version,
        "downloading": false,
...
fn apply_saved_main_position(app: &tauri::AppHandle) {
  ...
  let Some(pos) = st.window_pos else { return; };
...
fn apply_hotkey(app: &tauri::AppHandle, hotkey: &str) -> Result<(), String> {
  ...
  manager.unregister_all().map_err(|e| format!("unregister_all failed: {e}"))?;
  manager.register(hk.as_str()).map_err(|e| format!("register hotkey failed: {e}"))?;
  Ok(())
}
...
fn open_in_folder(file_path: String) -> bool {
  #[cfg(target_os = "macos")]
  {
    let _ = std::process::Command::new("open").args(["-R", &file_path]).spawn();
    return true;
  }
  #[cfg(target_os = "windows")]
  {
  ...
  if let Some(w) = app.get_webview_window("main") {
    if let (Ok(size), Ok(scale)) = (w.inner_size(), w.scale_factor()) {
      ...
      if target_h > current_h + 1.0 {
        let _ = w.set_size(tauri::Size::Logical(tauri::LogicalSize::new(width_logical, target_h)));
        ensure_main_window_in_screen(&app);
      }
    }
  }
...
fn open_settings(app: tauri::AppHandle) -> bool {
  let title = if is_ru(&app) { "Настройки" } else { "Settings" };
...
fn place_sort_window_below_main(app: &tauri::AppHandle) {
  ...
  let Some(sort) = app.get_webview_window("sort") else { return; };

  let Ok(main_pos) = main.outer_position() else { return; };
  let Ok(main_size) = main.outer_size() else { return; };
  let Ok(sort_size) = sort.outer_size() else { return; };
  ...
  let _ = sort.set_position(tauri::Position::Physical(tauri::PhysicalPosition::new(x, y)));
}

#[tauri::command]
fn open_sort_window(app: tauri::AppHandle) -> bool {
  let title = if is_ru(&app) { "Сортировка" } else { "Sorting" };
...
fn update_settings(app: tauri::AppHandle, partial: Value) -> Settings {
  ...
  if let Some(v) = partial.get("paused").and_then(|v| v.as_bool()) { current.paused = v; }
  if let Some(v) = partial.get("hotkey").and_then(|v| v.as_str()) { current.hotkey = v.to_string(); }
  if let Some(v) = partial.get("rememberQuery").and_then(|v| v.as_bool()) { current.remember_query = v; }
  if let Some(v) = partial.get("rememberPos").and_then(|v| v.as_bool()) { current.remember_pos = v; }
  if let Some(v) = partial.get("opacity").and_then(|v| v.as_f64()) { current.opacity = v; }
  if let Some(v) = partial.get("indexIntervalSec").and_then(|v| v.as_i64()) { current.index_interval_sec = v; }
  if let Some(v) = partial.get("maxFileSizeMb").and_then(|v| v.as_i64()) { current.max_file_size_mb = v; }
  if let Some(v) = partial.get("unlimitedIndexing").and_then(|v| v.as_bool()) { current.unlimited_indexing = v; }
  if let Some(v) = partial.get("scheduleEnabled").and_then(|v| v.as_bool()) { current.schedule_enabled = v; }
  if let Some(v) = partial.get("scheduleMinutes").and_then(|v| v.as_i64()) { current.schedule_minutes = v.max(1); }
  if let Some(v) = partial.get("aiModel").and_then(|v| v.as_str()) { current.ai_model = v.to_string(); }
  if let Some(v) = partial.get("language").and_then(|v| v.as_str()) {
    current.language = if v.eq_ignore_ascii_case("ru") { "ru".to_string() } else { "en".to_string() };
  }
  ...
  if let Some(v) = partial.get("licenseEmail").and_then(|v| v.as_str()) { current.license_email = v.to_string(); }
  if let Some(v) = partial.get("licenseKey").and_then(|v| v.as_str()) { current.license_key = v.to_string(); }
  if let Some(v) = partial.get("licenseStatus").and_then(|v| v.as_str()) { current.license_status = v.to_string(); }
  ...
  let _ = app.emit("settings-updated", serde_json::to_value(&current).unwrap_or_else(|_| json!({})));
  current
}
...
fn search(app: tauri::AppHandle, state: State<'_, AppState>, query: String) -> Vec<Value> {
  ...
  for (_k, v) in index.files {
    ...
    let mut score = 0;
    if nl.contains(&q) { score += 3; }
    if pl.contains(&q) { score += 2; }
    if tl.contains(&q) { score += 1; }
    if score == 0 { continue; }
...
fn get_ai_status(state: State<'_, AppState>) -> Value {
  state.ai_status.lock().map(|s| s.clone()).unwrap_or_else(|_| json!({
    "installed": false,
    "installing": false,
    "model": "qwen2.5:1.5b",
    "progress": "",
...
fn install_ai(app: tauri::AppHandle, state: State<'_, AppState>) -> Value {
  ...
  let txt_preparing = if is_ru { "Подготовка установки..." } else { "Preparing installation..." };
  let txt_no_ollama = if is_ru { "Ollama не найдена. Установи Ollama и повтори." } else { "Ollama was not found. Install Ollama and try again." };
  let txt_loading = if is_ru { "Загрузка модели..." } else { "Downloading model..." };
  let txt_installed = if is_ru { "ИИ установлен" } else { "AI installed" };
  let txt_install_error = if is_ru { "Ошибка установки ИИ" } else { "AI installation error" };

  {
    if let Ok(mut s) = state.ai_status.lock() {
      let installing = s.get("installing").and_then(|x| x.as_bool()).unwrap_or(false);
      if installing {
        return s.clone();
      }
  ...
  if code == 0 {
    ...
    if let Ok(mut s) = state.ai_status.lock() {
      s["installing"] = Value::Bool(false);
      s["installed"] = Value::Bool(false);
      s["error"] = Value::String(if msg.trim().is_empty() { txt_install_error.to_string() } else { msg.trim().to_string() });
      let _ = app.emit("ai-status", s.clone());
    }
  }
...
fn remove_ai(app: tauri::AppHandle, state: State<'_, AppState>) -> Value {
  ...
  let lang = load_settings_internal(&app).language;
  let is_ru = lang.eq_ignore_ascii_case("ru");
  let txt_remove_error = if is_ru { "Не удалось удалить модель" } else { "Failed to remove model" };
  let (code, stdout, stderr) = run_command_capture("ollama", &["rm", &model]);

  if code != 0 && code != -1 {
    if let Ok(mut s) = state.ai_status.lock() {
      let msg = if !stderr.trim().is_empty() { stderr } else { stdout };
      s["error"] = Value::String(if msg.trim().is_empty() { txt_remove_error.to_string() } else { msg.trim().to_string() });
      let _ = app.emit("ai-status", s.clone());
    }
    save_ai_status_internal(&app, state.inner());
    return get_ai_status(state);
...
fn get_duplicate_result(state: State<'_, AppState>) -> Value {
  state.duplicate_result.lock().map(|v| v.clone()).unwrap_or_else(|_| json!({
    "mode": "duplicates",
    "groups": [],
    "total": 0,
    "copies": 0
...
#[tauri::command]
fn start_duplicate_sort(app: tauri::AppHandle, state: State<'_, AppState>, mode: Option<String>) -> Value {
  let ru = is_ru(&app);
  ...
  let requested_mode = mode.unwrap_or_else(|| "duplicates".to_string()).to_lowercase();
  ...
  let total = items.len();
  let _ = app.emit("sort-progress", json!({"type":"scan","processed":0,"total":total}));
  ...
  if organize_mode {
    ...
    for (i, item) in items.iter().enumerate() {
      ...
      if source_path.exists() {
        ...
        let ext_label = match ext.as_str() {
          ...
          ".xls" | ".xlsx" => "Excel",
          ".pdf" => "PDF",
          _ => if ru { "Документы" } else { "Documents" },
        };
      ...
      if i % 80 == 0 || i + 1 == total {
        let _ = app.emit("sort-progress", json!({"type":"scan","processed":i + 1,"total":total}));
      }
    }
    ...
    let moved_total: usize = groups
      .iter()
      .map(|g| g.get("copies").and_then(|x| x.as_array()).map(|a| a.len()).unwrap_or(0))
      .sum();
    ...
    let _ = app.emit("sort-progress", json!({"type":"done","processed":total,"total":total}));
  ...
  for (i, item) in items.iter().enumerate() {
    ...
    let name_key = name.chars().filter(|c| c.is_alphanumeric()).collect::<String>();
    let ext = get_value_str(item, "ext").to_lowercase();
    let text = get_value_str(item, "text").to_lowercase();
    let text_key = text.chars().filter(|c| c.is_alphanumeric() || c.is_whitespace()).take(4000).collect::<String>();
  ...
  for (_k, mut list) in map {
    ...
    let copies: Vec<Value> = list.into_iter().map(|x| json!({
  ...
  let copies_total: usize = groups.iter().map(|g| g.get("copies").and_then(|x| x.as_array()).map(|a| a.len()).unwrap_or(0)).sum();
  let payload = json!({
    "mode": "duplicates",
    "groups": groups,
    "total": total,
...
fn delete_duplicate_files(app: tauri::AppHandle, paths_to_delete: Vec<String>) -> Value {
  ...
  let unique: Vec<String> = {
    let mut seen = std::collections::HashSet::new();
    paths_to_delete.into_iter().filter(|p| !p.trim().is_empty() && seen.insert(p.clone())).collect()
  };
  ...
  for p in &unique {
    let pb = PathBuf::from(p);
    if !pb.exists() {
      failed.push(json!({"path": p, "error": if ru { "Файл не найден" } else { "File not found" }}));
      continue;
    }
    if pb.is_dir() {
      failed.push(json!({"path": p, "error": if ru { "Это не файл" } else { "This is not a file" }}));
      continue;
    }
  ...
  if !deleted.is_empty() {
    ...
    if let Some(ip) = index_path(&app) {
      ensure_parent(&ip);
      let _ = fs::write(ip, serde_json::to_string_pretty(&index).unwrap_or_else(|_| "{}".to_string()));
    }
    if let Some(st) = app.try_state::<AppState>() {
...
#[tauri::command]
fn ask_ai_blocking(app: tauri::AppHandle, state: AppState, query: String, file_paths: Vec<String>) -> Value {
  let ru = is_ru(&app);
  ...
  let attached: Vec<Value> = file_paths.iter().take(8).map(|p| read_user_file_context(p)).collect();
...
",
      q,
      if context_block.is_empty() {"[empty]"} else {&context_block},
      if attached_block.is_empty() {"[none]"} else {&attached_block}
    )
  };
  ...
  if !use_model {
    let mut lines: Vec<String> = vec![if is_ru { "Локальная модель сейчас недоступна, но я нашел это в проиндексированных файлах:" } else { "Local model is currently unavailable, but I found this in indexed files:" }.to_string()];
...
#[tauri::command]
async fn ask_ai(app: tauri::AppHandle, query: String, file_paths: Vec<String>) -> Result<Value, String> {
  let ru = is_ru(&app);
  let st = app.state::<AppState>().inner().clone();
  let out = tauri::async_runtime::spawn_blocking(move || ask_ai_blocking(app, st, query, file_paths))
    .await
    .map_err(|e| if ru { format!("Ошибка ИИ: {}", e) } else { format!("AI error: {}", e) })?;
  Ok(out)
}
...
fn get_recommended_model() -> String {
  let cpu = std::thread::available_parallelism().map(|x| x.get()).unwrap_or(1);
  if cpu <= 4 { "qwen2.5:1.5b".to_string() } else { "qwen2.5:3b".to_string() }
}
...
pub fn run() {
  tauri::Builder::default()
    ...
    .setup(|app| {
      ...
      let _ = app.emit("index-status", initial);
...

      {
        ...
        std::thread::spawn(move || loop {
          ...
          if app_state.worker.lock().ok().and_then(|w| w.as_ref().cloned()).is_some() {
            continue;
          }
          let now = SystemTime::now().duration_since(UNIX_EPOCH).unwrap_or_default().as_secs() as i64;
...

      {
        ...
        std::thread::spawn(move || loop {
          ...
          let _ = tauri::async_runtime::block_on(check_app_update_internal(&app_handle, &app_state, false));
        });
      }
